...
export default function MapScreen({ state, monthKey, today, onSubmitTasks, onToggleGoal, onDirty, draftReset }) {
  ...
  const selectedDayPoints = state.tasks.filter(task => task.daily && selected.has(task.id)).reduce((sum, task) => sum + task.points, 0);
  ...
  return <div className="screen-content"><div className="screen-heading"><h2>冒险地图</h2><select className="month-select" aria-label="查看月份" value={viewMonth} onChange={event => { if (dirty && !window.confirm("放弃未提交的修改并切换月份？")) return; setViewMonth(event.target.value); setSelectedDay(null); setDirty(false); onDirty(false); }}>{options.map(key => <option key={key}>{key}</option>)}</select></div>
    ...
    {selectedDay && <section className="day-editor"><h3>{selectedDay} · {selectedDayPoints} ★</h3><p className="schedule-note">{selectedDay < dateKeyFromDate(today) ? "补记或更正" : "今日记录"}</p>{state.months[viewMonth]?.dayMeta?.[selectedDay]?.backfilled && <p className="schedule-note">此日期有补记记录</p>}{state.tasks.filter(task => task.daily && (task.enabled || days[selectedDay]?.[task.id])).map(task => <TaskCard key={task.id} task={task} checked={selected.has(task.id)} detail={rewardRule(task)} onToggle={() => toggle(task.id)} />)}<button className="primary-button" disabled={!dirty} onClick={save}>保存这一天</button><p role="status" className="schedule-note">{message}</p></section>}
    ...
    <section className="section-block"><div className="section-title"><h3>每日星星记录</h3><span>本月 {summary.earnedPoints} ★</span></div><p className="schedule-note">每个打卡日独立计分，不要求连续打开或连续完成。当天完成任务后，星星会立即加入累计余额。</p>{state.tasks.filter(task => task.enabled && task.daily).map(task => <div className="goal-row" key={task.id}><span className={summary.taskStatus[task.id] ? "goal-icon achieved" : "goal-icon"}>{summary.taskStatus[task.id] ? "✓" : "·"}</span><span><strong>{task.label}</strong><small>{rewardRule(task)}</small></span><b>累计{summary.counts[task.id]}次 · {summary.counts[task.id] * task.points}星</b></div>)}</section>
  </div>;
}
