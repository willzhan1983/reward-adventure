...
export default function TodayScreen({ state, monthKey, summary, today, onSubmitTasks, onToggleGoal }) {
  const todayKey = dateKeyFromDate(today);
  const record = state.months[monthKey]?.days?.[todayKey] || {};
  const savedTaskIds = Object.keys(record).filter((taskId) => record[taskId]).sort().join(",");
